...
MODEL_NAME AS CELLPHONE
FROM
FACT_TRANSACTIONS A
LEFT JOIN DIM_MODEL B ON A.IDModel=B.IDModel
LEFT JOIN DIM_DATE  C  ON A.Date=C.Date
LEFT JOIN DIM_LOCATION D ON A.IDLocation=D.IDLocation
WHERE YEAR >= 2005 AND MODEL_NAME NOT IN ('6230 (6233)', 'E1100', 'E250', '3210','5230','6600','3310 (3330)', '6010 (6020/6030)','C139', 'C200', 'Droid Bionic', 'RAZR V3','THUNDERBOLT')
...
STATE AS CUSTOMER_STATE,
COUNTRY AS CUSTOMER_COUNTRY,
COUNT(MODEL_NAME) AS CNT_OF_CELLPHONES
FROM
FACT_TRANSACTIONS A
LEFT JOIN DIM_MODEL B ON A.IDModel=B.IDModel
LEFT JOIN DIM_DATE  C  ON A.Date=C.Date
LEFT JOIN DIM_LOCATION D ON A.IDLocation=D.IDLocation
WHERE  MODEL_NAME LIKE 'GALAXY%' AND COUNTRY= 'US'
GROUP BY
COUNTRY, STATE

--ARIZONA, CALIFORNIA AND MARYLAND ARE BUYING MORE SAMSUNG CELLPHONES
------------------------------------------------------------------------------------------------------------------------------------------------
...
FACT_TRANSACTIONS A
LEFT JOIN DIM_MODEL B ON A.IDModel=B.IDModel
LEFT JOIN DIM_LOCATION C ON A.IDLocation=C.IDLocation
GROUP BY
...
FACT_TRANSACTIONS A
LEFT JOIN DIM_MODEL B ON A.IDModel=B.IDModel
WHERE MODEL_NAME NOT IN ('6230 (6233)', 'E1100', 'E250', '3210','5230','6600','3310 (3330)', '6010 (6020/6030)','C139', 'C200', 'Droid Bionic', 'RAZR V3','THUNDERBOLT')
GROUP BY
MODEL_NAME, TOTALPRICE
ORDER BY
TOTALPRICE

--CHEAPEST CELLPHONE IS ONE PLUS X
...
SELECT TOP 5
MODEL_NAME AS MODEL,
AVG(TotalPrice) AS AVER_MODEL_PRICE,
...
GROUP BY
MODEL_NAME, QUANTITY, MANUFACTURER_NAME
ORDER BY
QUANTITY DESC

------------------------------------------------------------------------------------------------------------------------------------------------

--Q6. LIST THE NAMES OF CUSTOMERS AND THE AVERAGE AMOUNT SPENT IN 2009, WHERE THE AVERAGE IS HIGHER THAN 500.


SELECT 
Customer_Name AS CUSTOMER_NAME,
AVG(TotalPrice) AS AVER_AMOUNT
FROM
FACT_TRANSACTIONS A
INNER JOIN DIM_CUSTOMER B ON A.IDCustomer=B.IDCustomer
INNER JOIN DIM_DATE C ON A.Date=C.DATE
WHERE YEAR='2009'
GROUP BY
Customer_Name
HAVING
...
--DROID BLANC WAS IN THE TOP 5 IN TERMS OF QUANTITY AND SALES IN ALL THE THREE YEARS
...
-- USE ROW NO. AND USE RANKING AND FILTER DATA

SELECT * FROM 
(
SELECT 
ROW_NUMBER() OVER( ORDER BY TOTALPRICE DESC ) AS RNUM, 
MANUFACTURER_NAME AS MANUFACTURER_NAME,
DATEPART(YEAR, DATE) AS PURCHASE_YEAR
FROM 
FACT_TRANSACTIONS A
LEFT JOIN DIM_MODEL B ON A.IDModel=B.IDModel
LEFT JOIN DIM_MANUFACTURER C ON B.IDManufacturer=C.IDManufacturer
...
--APPLE IS THE MANUFACTURERS WITH 2ND TOP SALES IN 2009 AND SAMSUNG IS THE MANUFACTURER WITH 2ND TOP SALES IN 2010.
-------------------------------------------------------------------------------------------------------------------------------------------------

--Q9. SHOW THE MANUFACTURERS THAT SOLD THE CELLPHONE IN 2010 BUT DIDNT IN 2009?
...
-- HTC SOLD THE CELLPHONE IN 2010 BUT NOT IN 2009
...
SELECT 
CUSTOMER_NAME AS CUSTOMER_NAME,
CASE WHEN YEAR_ORDER  =2003  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2003,
CASE WHEN YEAR_ORDER =2004  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2004,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2005,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2006,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2007,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2008,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2009,
CASE WHEN YEAR_ORDER =2005  THEN AVERAGE_SPEND END AS AVERAGE_SPEND_2010
FROM
(
SELECT  DISTINCT TOP 100
CUSTOMER_NAME AS CUSTOMER_NAME,
DATEPART(YEAR, DATE) AS YEAR_ORDER, 
AVG(TOTALPRICE) AS AVERAGE_SPEND,
AVG(QUANTITY) AS AVERAGE_QUANTITY
FROM 
FACT_TRANSACTIONS A
LEFT JOIN DIM_CUSTOMER B ON A.IDCustomer=B.IDCustomer
WHERE DATEPART(YEAR, DATE)='2003'
GROUP BY
CUSTOMER_NAME, DATEPART(YEAR, DATE)
UNION ALL
SELECT DISTINCT TOP 100
CUSTOMER_NAME AS CUSTOMER_NAME,
DATEPART(YEAR, DATE) AS YEAR_ORDER,
...
--CREATE A CROSS TABLE AND USE CASE WHEN- MANUAL QUESTIONS